# urdr Promotion — Shared Source of Truth (`pitch.md`)

> **This is the single source of truth for *what urdr is and how we talk about it*.**
...
| Field | Value |
|---|---|
| Name | **urdr** — never `pikiclaw` (renamed; the npm `pikiclaw` package is only a forwarding shim, do not advertise it) |
| Install | `npx urdr@latest` |
...
OpenAI-compatible). Self-bootstrapped — urdr is built with urdr.

---

## 1. Orienter (one compact line — assume the reader has never heard of urdr)
...
- **zh:** “我自己也在做一个类似的东西 `urdr`——一个 agent orchestrator，把 Claude Code / Codex / Gemini 接到 飞书 / Telegram / Slack / Discord 等 + 本地 dashboard，可并行跑多个 agent。”
- **en:** “I'm building a similar thing (`urdr`) — an agent orchestrator that runs Claude Code / Codex / Gemini across Telegram / Slack / Discord (and more) plus a localhost dashboard, several agents in parallel.”
...
urdr's **default** Claude driver runs the *real interactive Claude Code TUI* under a PTY, so turns
...
2. **Rough UX / complex setup** → "Single `npx urdr@latest` start, no config files, dashboard auto-opens."
...
- Only claim a capability urdr **actually ships today**. Readers will check. If unsure, drop the claim.
...
- **Disclosure is mandatory and is also our shield:** every draft says "I'm building urdr" (or 我在做 `urdr`). Undisclosed promotion is what gets flagged; disclosed peer-sharing is what platforms tolerate.
...
- **Never critique the host project.** No "switch from X", "X doesn't support", "while waiting for X to fix", "X 还没做" — even neutral-sounding versions telegraph competitive intent. urdr is *another option*, never a *replacement*.
...
1. **Peer-position + one-line orienter** (§1). Assume the reader arrived from a notification and has never heard of urdr. Fold "I hit the same thing" and the orienter into the *same* opener — never an ad-header paragraph.
2. **Out-of-box claim — the headline.** Answer the reader's real question — *"if I install this, do I get this without setup?"* — FIRST, pinned to the issue's exact pain. Not "urdr has lots of features" but "this exact capability is there the moment you run `npx urdr@latest`, no config edits / env vars / patches." This is the load-bearing sentence.
3. **One sentence of implementation evidence** (§4) — the credibility anchor. Exactly one sentence; not a tutorial.
4. Trial line: `npx urdr@latest`
...
| Link placement | **NEVER in the main reply** — main reply = pitch + `npx urdr@latest` (native, no penalty); **GitHub link goes in a self-reply** (X downranks link-bearing posts) | inline `npx … · github…` line, unless the sub bans links → no-link variant | inline at the end of the comment |
| Length | 1–2 sentences (≤3) | 2–4 sentences | ≤5 sentences |
| Disclosure | "building urdr" framing | "I'm building urdr" from the first line | "I'm building / 我在做 urdr" |
...
- Reads like marketing copy / vendor voice ("urdr is X", "urdr can do X", "check out", "you should try").
- Compares urdr to the host project, or implies the host is lacking.
- Opens with implementation detail before the reader knows what urdr is.
...
- Missing the disclosure line, the `npx urdr@latest`, or the repo link (where the channel requires it).
