...
`urdr` uses Vitest for all automated tests. Live runtime validation goes through `npm run dev`.
...
# Manual runtime / startup validation (preferred for anything that launches urdr)
npm run dev
...
If a test or validation step needs to launch `urdr` itself, use `npm run dev`.

- `npm run dev` is the local-only startup path
- It runs with `--no-daemon`, so it stays on the checked-out source tree
- It rewrites `~/.urdr/dev/dev.log` on each launch
- Do not kill or reuse the long-lived production/self-bootstrap `npx urdr@latest` process on this machine as part of dev testing
...
For any IM-driven manual end-to-end validation, start the local runtime with `npm run dev` and inspect `~/.urdr/dev/dev.log`.
