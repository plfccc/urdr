...
describe('_detectBrowserMcpFailure', () => {
  it('detects known failure patterns and ignores benign chunks', () => {
    ...
    const mcpLine = '{"type":"user","message":{"content":[{"type":"tool_result","content":"mcp__urdr-browser__browser_navigate: http://x failed: MCP error -32000: Connection closed"}]}}';
    expect(_detectBrowserMcpFailure(mcpLine)).toBe('urdr-browser MCP stdio closed');
