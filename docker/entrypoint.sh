#!/usr/bin/env bash
#
# urdr-entrypoint — container init for the urdr image.
#
...
#   3. exec the compiled urdr CLI with the user-supplied args.
#
...
URDR_DIR="${HOME_DIR}/.urdr"
WORKDIR_DEFAULT="${URDR_WORKDIR:-/workspace}"
...
mkdir -p "${URDR_DIR}" "${WORKDIR_DEFAULT}" \
         "${HOME_DIR}/.claude" "${HOME_DIR}/.codex" 2>/dev/null || true

# Banner — short enough not to spam, useful for `docker logs` triage.
echo "──────────────────────────────────────────────────────────────"
echo " urdr container starting"
echo "   HOME      : ${HOME_DIR}"
echo "   config    : ${URDR_DIR}"
...
# CMD declares. Keeps `docker run urdr bash` ergonomic without losing the
# "just run the bot" default for `docker run urdr`.
if [[ $# -eq 0 ]]; then
...
case "${1:-}" in
  bash|sh|node|npm|npx|claude|codex|urdr)
    exec "$@"
    ;;
esac
